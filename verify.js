...
// fit:0 — в эталоне натяга сверх компенсации реза не было (посадки ±kerf).
const ETALON = { W: 99.4, H: 99.4, D: 44.4, t: 3.0, acr: 2.1, gap: 2.2, kerf: 0.2, fit: 0, frame: 4.2, lip: 0.6 };
...
// Компенсация резаных торцов (tipComp по умолчанию): на kerf/2 (отступ кромки
// реза) удлинена всякая кромка, чей торец выходит на наружную поверхность
// коробки, — торцы сквозных шипов, нижние кромки стенок, передние/задние торцы
// боковых, все 4 кромки верха.
const mc = BG.model(ETALON);
const gc = (id) => mc.pieces.find((p) => p.id === id);
const e = ETALON.kerf / 2;
const TIP_CHECKS = [
